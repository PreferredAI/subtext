...
def output_examples(file_prefix, all_examples, num_samples=5, dedupe=True):
    ...
    for sample_idx in range(num_samples):
        ...
        test_idx = int((target_class_size * 0.8) // 1)
        logger.info(f'splitting into {test_idx} training examples ({test_idx/target_class_size} %) per class')
...
def main(args):
    ...
    for row in tqdm(data):
        tags = {i['term'] for i in eval(row['tag'])}
        if len(tags) > 1:
            continue
        else:
            tags = tags.pop()
        for sent in text_to_sentences(row['summary'].replace('\n', ' ')).split('\n'):
            ...
            for i in range(len(sent_toks)):
                ...
                if window_toks:
                    results[query].append((tags, pre, post))
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--input', type=Path, default=Path('raw/arXiv/arxivData.json'))
    argparser.add_argument('--out_dir', type=Path, default=Path('arXiv'))
    ...
    argparser.add_argument('--keep_punctuation', action='store_true')
